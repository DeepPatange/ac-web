...
import { siteConfig, contactCopy, contactMeta, cta, products } from "@/lib/site";

/* -------------------------------------------------------------------------- */
/*  Dark dotted-map card — the GlobalPresence visual language, one Mumbai pin. */
/*  Static (quiet section): no loops, no arcs — the node just sits and glows.  */
/* -------------------------------------------------------------------------- */

const MUMBAI = { lat: 19.076, lng: 72.8777 };

/** Same equirectangular projection GlobalPresence's map uses (800×400 box). */
...
/**
 * Reads a product slug from the URL. Supports both link shapes:
 *   #contact?product=<slug>   (Products-grid Enquire links)
 *   ?product=<slug>           (plain query string)
 */
function readProductSlugFromUrl(): string | null {
  if (typeof window === "undefined") return null;
  const { hash, search } = window.location;
  const q = hash.indexOf("?");
  const fromHash =
    q >= 0 ? new URLSearchParams(hash.slice(q + 1)).get("product") : null;
  const fromSearch = new URLSearchParams(search).get("product");
...
export default function Contact() {
  ...
  /* Pre-fill the product select from the URL — on mount and on every
     hashchange (the Products grid links to #contact?product=<slug>). Because
     "contact?product=…" is not a real element id, the browser cannot perform
     the anchor jump itself, so we scroll the section into view manually. */
  useEffect(() => {
    function apply(shouldScroll: boolean) {
      const slug = readProductSlugFromUrl();
    ...
    // On mount: pre-fill, and rescue the scroll if the page loaded with the
    // hash-style deep link (native anchoring will have failed silently).
    apply(/^#contact\?/.test(window.location.hash));

    const onHashChange = () => apply(/^#contact\?/.test(window.location.hash));
  ...
  return (
    <section
      ref={sectionRef}
      id="contact"
      aria-label="Contact Accord Chemical Corporation"
        ...
        <div className="mt-12 grid gap-8 sm:mt-14 lg:grid-cols-[1fr_1.1fr] lg:gap-12">
          ...
          >
            ...
            <motion.a
              ...
              className={cn(infoTile, "block p-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accord-red/60")}
            >
              <TopRule />
          ...
          >
            <form noValidate onSubmit={handleSubmit} className="grid gap-5">
              ...
              {/* Product / enquiry type — pre-fills from #contact?product=<slug> */}
              <div>
