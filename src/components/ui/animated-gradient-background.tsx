...
const AnimatedGradientBackground: React.FC<AnimatedGradientBackgroundProps> = ({
  ...
  useEffect(() => {
    ...
    const paint = (w: number) => {
      if (el)
        el.style.background = `radial-gradient(${w}% ${w + topOffset}% at 50% 20%, ${stopsString})`;
    };

    // Paint the gradient once up front.
    paint(startingGap);

    // Static gradient (the common case here) → no animation loop at all.
    if (!Breathing) return;
    ...
    // Only run the loop while the gradient is actually on screen.
    const start = () => {
